...
// (name) => header roast, list of missed items appended below
const MISSED_ROASTS = [
  (name) => `${name}, пидор ленивый, ты забыл сделать выбор:`,
  (name) => `Эй ${name}! Пока ты сосал хуи, все уже поставили. А ты нет, гомосек:`,
  (name) => `${name} — главный пидрила турнира. Забыл поставить:`,
  (name) => `${name}, жопошник, вытащи хуй изо рта и поставь:`,
  (name) => `${name}, очнись, сука! Ты не сделал выбор:`,
  (name) => `${name}, мразь ленивая, двигай жопой на сайт! Не поставил:`,
  (name) => `ПОЗОР! ${name} опять проебался. Забыл:`,
  (name) => `${name}, гейский позор! Руки заняты чем-то другим? Не поставил:`,
  (name) => `Опять ${name} в пидорской отключке. Не сделал выбор:`,
  (name) => `${name}, блять, ты чё совсем охуел? Все поставили, а ты нет:`,
  (name) => `${name} забыл про ставки. Вчера так набухался что мозги вытекли через жопу 🍸 Не поставил:`,
  (name) => `${name}, хорош дрочить, зайди на nba.nagaev.dev! Пропустил:`,
  (name) => `${name}, петух, тебя ждут ставки! Забыл сделать выбор:`,
  (name) => `${name}, пидорас, завтра будешь ныть что не успел. Не поставил:`,
  (name) => `${name}, гомосек, все уже поставили, только ты сидишь! Забыл:`,
];
...
async function checkMissedPredictions() {
  ...
  for (const { name, missed } of Object.values(missedByUser)) {
    ...
    messages.push(pick(MISSED_ROASTS)(name) + "\n" + list);
  }
